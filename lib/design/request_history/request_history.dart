              ...
              Row(
                children: const [
                  Expanded(
                    ...
                    children: [
                      _AttendanceWidget(
                        type: LeaveRequest.Pending,
                      ),
                      _AttendanceWidget(
                        type: LeaveRequest.Approved,
                      ),
                      _AttendanceWidget(
                        type: LeaveRequest.Rejected,
                      ),
                    ],
                  )
                      ),
...
class _AttendanceWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<Map<String, dynamic>> filteredList = context
        .select((RequestHistoryVm value) => value.requestList)
      ...
      child: ListTileTheme(
        child: ExpansionTile(
          ...
          subtitle: Text(
            position,
            style: context.textTheme.bodySmall
                ?.copyWith(fontSize: 10, color: colorScheme.onPrimary),
          ),
...
Expanded(
child: CusButton(text: 'Approved', onTap: () {})
    .paddingSymmetric(horizontal: 10)),
],
).paddingSymmetric(vertical: 5),*/
